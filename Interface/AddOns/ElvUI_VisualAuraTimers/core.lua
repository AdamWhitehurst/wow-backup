...
function VAT:CreateBar(button)
	if not button.Holder then
		...
		if E.db.VAT.useBackdropTexture then
			...
			BarHolder.pixelBorders.CENTER:SetTexture(texture)
		end
...
function VAT:UpdateHeader(header)
	...
	while (child) do
		if (child.Holder) then
			...
			if E.db.VAT.useBackdropTexture then
				local texture = LSM:Fetch("statusbar", E.db['VAT'].backdropTexture)
                child.Holder.pixelBorders.CENTER:SetTexture(texture)
			end
...
E.PopupDialogs.VAT_INCLUDED_IN_ELVUI = {
...
function VAT:Initialize()
	...
	if ((WOW_PROJECT_ID == WOW_PROJECT_MAINLINE and tonumber(E.version) >= 11.25) or (WOW_PROJECT_ID ~= WOW_PROJECT_MAINLINE and tonumber(E.version) >= 1.16)) then
		E:StaticPopup_Show("VAT_INCLUDED_IN_ELVUI")
		return
	end
