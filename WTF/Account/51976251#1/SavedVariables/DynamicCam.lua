
DynamicCamDB = {
	["profileKeys"] = {
		["Vedrixa - Moon Guard"] = "Default",
		["Honabar - Emerald Dream"] = "Default",
		["Andalsam - Tichondrius"] = "KUI",
		["Zandroh - Tichondrius"] = "Default",
		["Mahakah - Tichondrius"] = "Default",
		["Umbrick - Emerald Dream"] = "Default",
		["Byanir - Moon Guard"] = "Default",
		["Wharapi - Tichondrius"] = "Default",
		["Ayanir - Moon Guard"] = "Default",
		["Mcpokesalot - Tichondrius"] = "Default",
		["Iscrackl - Tichondrius"] = "Default",
		["Andallsa - Tichondrius"] = "Default",
		["Sparkwizz - Tichondrius"] = "Default",
		["Stoakt - Tichondrius"] = "Default",
		["Magnr - Tichondrius"] = "Default",
		["Zandro - Tichondrius"] = "Default",
		["Andalsa - Tichondrius"] = "Default",
		["Tirdailough - Tichondrius"] = "Default",
		["Curaney - Tichondrius"] = "Default",
		["Brungar - Tichondrius"] = "New",
		["Umbrick - Tichondrius"] = "New",
		["Mininimi - Moon Guard"] = "Default",
		["Bjorgr - Tichondrius"] = "Default",
		["Grandh - Tichondrius"] = "Default",
		["Arndoss - Tichondrius"] = "Default",
		["Bramblebrow - Emerald Dream"] = "Default",
		["Madylad - Tichondrius"] = "Default",
		["Harwapa - Tichondrius"] = "Default",
	},
	["profiles"] = {
		["KUI"] = {
			["version"] = 2,
			["advanced"] = true,
			["reactiveZoom"] = {
				["enabled"] = true,
			},
			["defaultVersion"] = 1,
			["situations"] = {
				["033"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["023"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["006"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomSetting"] = "set",
					},
				},
				["302"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomSetting"] = "set",
						["zoomFitToggleNameplate"] = true,
						["yawDegrees"] = -10,
						["rotateSetting"] = "degrees",
						["zoomValue"] = 5,
						["rotateBack"] = true,
					},
					["cameraCVars"] = {
						["test_cameraDynamicPitchBaseFovPad"] = 0.01,
						["test_cameraDynamicPitch"] = 1,
						["test_cameraOverShoulder"] = -1.5,
						["test_cameraDynamicPitchBaseFovPadDownScale"] = 0.35,
					},
				},
				["005"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomSetting"] = "in",
						["zoomFitToggleNameplate"] = true,
					},
				},
				["002"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomValue"] = 8,
						["zoomFitToggleNameplate"] = true,
						["zoomSetting"] = "in",
					},
				},
				["034"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["100"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomValue"] = 35,
						["zoomFitToggleNameplate"] = true,
						["zoomSetting"] = "out",
					},
					["cameraCVars"] = {
						["test_cameraDynamicPitch"] = 0,
						["test_cameraHeadMovementStrength"] = 0,
						["test_cameraOverShoulder"] = 0,
					},
				},
				["102"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
					["cameraCVars"] = {
						["test_cameraOverShoulder"] = 0,
						["test_cameraHeadMovementStrength"] = 0,
						["test_cameraDynamicPitch"] = 0,
					},
				},
				["050"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["200"] = {
					["extras"] = {
						["hideUI"] = true,
					},
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomValue"] = 4,
						["transitionTime"] = 10,
						["zoomSetting"] = "in",
						["zoomFitToggleNameplate"] = true,
						["yawDegrees"] = 360,
						["rotateSetting"] = "degrees",
						["rotate"] = true,
						["timeIsMax"] = false,
					},
					["cameraCVars"] = {
						["test_cameraDynamicPitch"] = 0,
						["test_cameraHeadMovementStrength"] = 0,
						["test_cameraOverShoulder"] = 0,
					},
				},
				["301"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomValue"] = 4,
						["zoomFitToggleNameplate"] = true,
						["zoomSetting"] = "in",
					},
				},
				["060"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["031"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["201"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
					["cameraCVars"] = {
						["test_cameraDynamicPitch"] = 0,
						["test_cameraHeadMovementStrength"] = 0,
						["test_cameraOverShoulder"] = 0,
					},
				},
				["061"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["001"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomMin"] = 10,
						["zoomSetting"] = "range",
						["zoomFitToggleNameplate"] = true,
					},
				},
				["024"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["051"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["004"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomMin"] = 15,
						["zoomSetting"] = "range",
						["zoomFitToggleNameplate"] = true,
					},
				},
				["300"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomFitIncrements"] = 0.5,
						["zoomFitToggleNameplate"] = true,
						["zoomMax"] = 30,
						["zoomFitPosition"] = 90,
						["zoomMin"] = 3,
						["zoomSetting"] = "fit",
						["zoomValue"] = 4,
					},
					["cameraCVars"] = {
						["test_cameraDynamicPitch"] = 1,
						["test_cameraTargetFocusInteractEnable"] = 1,
						["test_cameraTargetFocusEnemyEnable"] = 1,
					},
				},
				["020"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["030"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["021"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["101"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomValue"] = 39,
						["zoomFitToggleNameplate"] = true,
						["zoomSetting"] = "set",
					},
					["cameraCVars"] = {
						["test_cameraHeadMovementStrength"] = 0,
						["test_cameraOverShoulder"] = 0,
					},
				},
			},
			["firstRun"] = false,
		},
		["Default"] = {
			["defaultCvars"] = {
				["test_cameraTargetFocusInteractEnable"] = 1,
				["test_cameraTargetFocusEnemyEnable"] = 1,
				["cameraZoomSpeed"] = 50,
				["test_cameraHeadMovementStandingDampRate"] = 50,
				["test_cameraHeadMovementDeadZone"] = 0,
				["test_cameraTargetFocusEnemyStrengthYaw"] = 0.1,
				["test_cameraHeadMovementRangeScale"] = 50,
				["test_cameraHeadMovementStrength"] = 2,
				["test_cameraHeadMovementMovingDampRate"] = 10.01,
				["test_cameraTargetFocusInteractStrengthYaw"] = 0.1,
				["test_cameraHeadMovementStandingStrength"] = 0.38,
				["test_cameraTargetFocusEnemyStrengthPitch"] = 0.1,
				["test_cameraDynamicPitch"] = 1,
				["test_cameraTargetFocusInteractStrengthPitch"] = 0.1,
			},
			["reactiveZoom"] = {
				["enabled"] = true,
				["addIncrements"] = 5,
				["maxZoomTime"] = 0.1,
				["addIncrementsAlways"] = 0,
				["incAddDifference"] = 2,
			},
			["advanced"] = true,
			["easingPitch"] = "Linear",
			["version"] = 2,
			["situations"] = {
				["033"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["pitchDegrees"] = 20,
						["zoomMax"] = 29,
						["zoomMin"] = 19,
						["transitionTime"] = 0.1,
						["zoomValue"] = 19,
						["rotateSetting"] = "degrees",
						["zoomSetting"] = "fit",
					},
					["cameraCVars"] = {
						["test_cameraOverShoulder"] = 0,
					},
					["view"] = {
						["enabled"] = true,
						["restoreView"] = true,
					},
				},
				["023"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["pitchDegrees"] = 15,
						["zoomMax"] = 19,
						["zoomMin"] = 19,
						["transitionTime"] = 0.1,
						["zoomSetting"] = "set",
						["rotateSetting"] = "degrees",
						["zoomValue"] = 15,
					},
					["cameraCVars"] = {
						["test_cameraOverShoulder"] = 0,
					},
					["view"] = {
						["enabled"] = true,
						["restoreView"] = true,
					},
				},
				["006"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["pitchDegrees"] = 20,
						["zoomValue"] = 19,
						["transitionTime"] = 0.1,
						["zoomSetting"] = "in",
						["rotateSetting"] = "degrees",
					},
					["cameraCVars"] = {
						["test_cameraOverShoulder"] = 0,
					},
					["view"] = {
						["enabled"] = true,
						["restoreView"] = true,
					},
				},
				["302"] = {
					["enabled"] = false,
				},
				["002"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["pitchDegrees"] = -20,
						["zoomSetting"] = "set",
						["zoomValue"] = 7,
						["rotateSetting"] = "degrees",
					},
					["cameraCVars"] = {
						["test_cameraDynamicPitchBaseFovPad"] = 0.05,
						["test_cameraDynamicPitch"] = 1,
						["test_cameraDynamicPitchBaseFovPadDownScale"] = 0.3,
						["test_cameraDynamicPitchSmartPivotCutoffDist"] = 12,
						["test_cameraDynamicPitchBaseFovPadFlying"] = 0.05,
					},
					["view"] = {
						["restoreView"] = true,
						["enabled"] = true,
					},
				},
				["031"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["pitchDegrees"] = 20,
						["zoomValue"] = 39,
						["transitionTime"] = 0.1,
						["zoomSetting"] = "set",
						["rotateSetting"] = "degrees",
					},
				},
				["004"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["pitchDegrees"] = 20,
						["zoomSetting"] = "set",
						["transitionTime"] = 0.1,
						["zoomValue"] = 39,
						["rotateSetting"] = "degrees",
					},
					["view"] = {
						["enabled"] = true,
						["restoreView"] = true,
					},
				},
				["102"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomValue"] = 39,
						["transitionTime"] = 0.3,
						["zoomSetting"] = "set",
					},
					["view"] = {
						["restoreView"] = true,
						["enabled"] = true,
					},
				},
				["050"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomSetting"] = "set",
						["zoomValue"] = 15,
					},
					["cameraCVars"] = {
						["test_cameraOverShoulder"] = 1.5,
					},
				},
				["030"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["pitchDegrees"] = 20,
						["zoomSetting"] = "in",
						["zoomValue"] = 19,
						["rotateSetting"] = "degrees",
					},
					["cameraCVars"] = {
						["test_cameraOverShoulder"] = 0,
					},
				},
				["301"] = {
					["enabled"] = false,
				},
				["005"] = {
					["cameraActions"] = {
						["pitchDegrees"] = -20,
						["transitionTime"] = 0.1,
						["rotateSetting"] = "degrees",
						["zoomSetting"] = "set",
					},
					["view"] = {
						["restoreView"] = true,
						["enabled"] = true,
					},
					["cameraCVars"] = {
						["test_cameraDynamicPitchBaseFovPad"] = 0.05,
						["test_cameraDynamicPitch"] = 1,
						["test_cameraDynamicPitchBaseFovPadDownScale"] = 0.3,
						["test_cameraDynamicPitchBaseFovPadFlying"] = 0.05,
						["test_cameraDynamicPitchSmartPivotCutoffDist"] = 12,
					},
				},
				["101"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomSetting"] = "set",
						["transitionTime"] = 0.3,
						["zoomValue"] = 39,
					},
					["view"] = {
						["restoreView"] = true,
						["enabled"] = true,
					},
				},
				["021"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["pitchDegrees"] = 15,
						["zoomSetting"] = "set",
						["transitionTime"] = 0.1,
						["zoomValue"] = 39,
						["rotateSetting"] = "degrees",
					},
				},
				["020"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["pitchDegrees"] = 15,
						["zoomValue"] = 14,
						["zoomSetting"] = "set",
						["rotateSetting"] = "degrees",
					},
					["cameraCVars"] = {
						["test_cameraOverShoulder"] = 0,
					},
					["view"] = {
						["enabled"] = true,
						["restoreView"] = true,
					},
				},
				["001"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["pitchDegrees"] = 25,
						["zoomSetting"] = "set",
						["transitionTime"] = 0.1,
						["zoomValue"] = 30,
						["rotateSetting"] = "degrees",
					},
				},
				["024"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["pitchDegrees"] = 15,
						["zoomValue"] = 15,
						["transitionTime"] = 0.1,
						["zoomSetting"] = "set",
						["rotateSetting"] = "degrees",
					},
					["cameraCVars"] = {
						["test_cameraOverShoulder"] = 0,
					},
					["view"] = {
						["enabled"] = true,
						["restoreView"] = true,
					},
				},
				["051"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomSetting"] = "set",
						["transitionTime"] = 0.1,
						["zoomValue"] = 15,
					},
					["cameraCVars"] = {
						["test_cameraOverShoulder"] = 0,
					},
					["view"] = {
						["enabled"] = true,
						["restoreView"] = true,
					},
				},
				["061"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomValue"] = 30,
						["transitionTime"] = 0.1,
						["zoomSetting"] = "set",
					},
					["view"] = {
						["enabled"] = true,
						["restoreView"] = true,
					},
				},
				["300"] = {
					["enabled"] = false,
				},
				["200"] = {
					["cameraActions"] = {
						["zoomValue"] = 39,
						["zoomSetting"] = "set",
						["rotate"] = true,
					},
				},
				["100"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomValue"] = 39,
						["transitionTime"] = 0.3,
						["zoomSetting"] = "set",
					},
					["view"] = {
						["restoreView"] = true,
					},
				},
				["034"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["pitchDegrees"] = 20,
						["zoomSetting"] = "in",
						["transitionTime"] = 0.1,
						["zoomValue"] = 19,
						["rotateSetting"] = "degrees",
					},
					["cameraCVars"] = {
						["test_cameraOverShoulder"] = 0,
					},
					["view"] = {
						["enabled"] = true,
						["restoreView"] = true,
					},
				},
				["060"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomValue"] = 39,
						["transitionTime"] = 0.1,
						["zoomSetting"] = "set",
					},
				},
			},
			["firstRun"] = false,
			["easingYaw"] = "Linear",
		},
		["New"] = {
			["defaultCvars"] = {
				["test_cameraHeadMovementRangeScale"] = 50,
				["test_cameraHeadMovementStrength"] = 2,
				["test_cameraDynamicPitchBaseFovPadDownScale"] = 0.7,
				["test_cameraHeadMovementMovingDampRate"] = 50,
				["test_cameraHeadMovementStandingStrength"] = 2,
				["test_cameraDynamicPitchBaseFovPad"] = 0.33,
				["test_cameraDynamicPitch"] = 1,
				["test_cameraHeadMovementMovingStrength"] = 0.25,
				["test_cameraDynamicPitchBaseFovPadFlying"] = 0.2,
				["test_cameraDynamicPitchSmartPivotCutoffDist"] = 12,
			},
			["version"] = 2,
			["advanced"] = true,
			["reactiveZoom"] = {
				["enabled"] = true,
			},
			["defaultVersion"] = 1,
			["situations"] = {
				["033"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["023"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["006"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomSetting"] = "set",
					},
				},
				["302"] = {
					["cameraActions"] = {
						["zoomSetting"] = "set",
						["zoomValue"] = 5,
						["zoomFitToggleNameplate"] = true,
						["yawDegrees"] = -10,
						["rotateSetting"] = "degrees",
						["zoomMax"] = 20,
						["rotateBack"] = true,
					},
					["cameraCVars"] = {
						["test_cameraDynamicPitchBaseFovPad"] = 0.01,
						["test_cameraDynamicPitch"] = 1,
						["test_cameraOverShoulder"] = -1.5,
						["test_cameraDynamicPitchBaseFovPadDownScale"] = 0.35,
					},
				},
				["005"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomSetting"] = "in",
						["zoomFitToggleNameplate"] = true,
					},
				},
				["002"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
						["zoomValue"] = 8,
						["zoomSetting"] = "in",
					},
				},
				["031"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["100"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
						["zoomValue"] = 35,
						["zoomSetting"] = "out",
					},
				},
				["102"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
					["cameraCVars"] = {
						["test_cameraOverShoulder"] = 0,
						["test_cameraDynamicPitch"] = 0,
					},
				},
				["050"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["200"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomValue"] = 4,
						["transitionTime"] = 10,
						["zoomFitToggleNameplate"] = true,
						["zoomSetting"] = "in",
						["yawDegrees"] = 360,
						["rotate"] = true,
						["rotateSetting"] = "degrees",
						["timeIsMax"] = false,
					},
					["cameraCVars"] = {
						["test_cameraDynamicPitch"] = 0,
						["test_cameraHeadMovementStrength"] = 0,
						["test_cameraOverShoulder"] = 0,
					},
					["extras"] = {
						["hideUI"] = true,
					},
				},
				["301"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
						["zoomValue"] = 4,
						["zoomSetting"] = "in",
					},
				},
				["060"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["101"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
						["zoomValue"] = 39,
						["zoomSetting"] = "set",
					},
					["cameraCVars"] = {
						["test_cameraOverShoulder"] = 0,
					},
				},
				["201"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
					["cameraCVars"] = {
						["test_cameraDynamicPitch"] = 0,
						["test_cameraHeadMovementStrength"] = 0,
						["test_cameraOverShoulder"] = 0,
					},
				},
				["061"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["001"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomMin"] = 10,
						["zoomSetting"] = "range",
						["zoomFitToggleNameplate"] = true,
					},
				},
				["024"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["051"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["004"] = {
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomMin"] = 15,
						["zoomSetting"] = "range",
						["zoomFitToggleNameplate"] = true,
					},
				},
				["300"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomFitIncrements"] = 0.5,
						["zoomMax"] = 30,
						["zoomSetting"] = "fit",
						["zoomFitPosition"] = 90,
						["zoomMin"] = 3,
						["zoomFitToggleNameplate"] = true,
						["zoomValue"] = 4,
					},
					["cameraCVars"] = {
						["test_cameraDynamicPitch"] = 1,
						["test_cameraTargetFocusInteractEnable"] = 1,
						["test_cameraTargetFocusEnemyEnable"] = 1,
					},
				},
				["020"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["030"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["021"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
				["034"] = {
					["enabled"] = false,
					["cameraActions"] = {
						["zoomMax"] = 20,
						["zoomFitToggleNameplate"] = true,
					},
				},
			},
			["firstRun"] = false,
		},
	},
}
